...
int main(int argc, char** argv)
{
  ...
  char** line;
  char b_line[longest_word_length];
  line = calloc(num_line, sizeof(char*));
  while(fgets(b_line,longest_word_length+2,fp))
  {
    num_line +=1;
  }
  fseek(fp, 0, SEEK_SET);
  while(fgets(b_line,longest_word_length+2,fp))
  {
    ...
    b_line[strlen(b_line)-1] = '\0';
    line[k] = calloc(strlen(b_line)+1,sizeof(char));
    strcpy(line[k],b_line);
    k++;
  }
         ...
         // printf("Let's start playing, %sThere are %d player(s) playing. The secret word is %d letter(s).",buffer,num_client,word_length);
          ...
          else
          {
            ...
            if(poi_cor == strlen(secret_word))
            {
              ...
              secret_word = line[rand()%k];
              secret_word[strlen(secret_word)] = '\0';
              word_length = strlen(secret_word);
            }
            else if(strlen(buffer) != (strlen(secret_word)))
            {
              sprintf(msg,"Invalid guess length. The secret word is %ld letter(s).\n",strlen(secret_word));
              printf("%s",msg);
              n = send(fd, msg,strlen(msg),0);
